...
class PatternDetector:
    ...
    def detect_spike(
        self, machine_id: str, sensor: str, current_value: float
        ...
        """
        if (
            sensor not in self.history[machine_id]
            or len(self.history[machine_id][sensor]) < 3
        ):
        ...
        is_spike = (
            z_score > 4.0
        )  # Increased from 3.0 to reduce false alarms in simulator
    ...
    def detect_pattern_anomaly(self, machine_id: str, data: Dict):
        ...
        for sensor, value in data.items():
            if sensor in SENSOR_CONFIG and value is not None:
                is_spike, score, reason = self.detect_spike(
                    machine_id, sensor, float(value)
                )
                ...
                if is_spike:
                    ...
                    result["confidence"] = max(
                        result["confidence"], min(score / 5, 1.0)
                    )
...
class AnomalyDetector:
    ...
    def _load_model(self):
        ...
        try:
            ...
            if model_path.exists() and scaler_path.exists():
                ...
                logger.warning(
                    f"Model files not found at {model_path}, using fallback detection"
                )
    ...
    def detect_anomaly(self, data: Dict, timestamp: str = None) -> Dict:
        ...
        try:
            ...
            # Pattern detection
            pattern_result = self.pattern_detector.detect_pattern_anomaly(
                machine_id, data
            )
            ...
            final_anomaly = (
                pattern_result["anomaly_detected"] or ml_result["anomaly_detected"]
            )
            ...
            return {
                ...
                "anomaly_types": list(
                    set(pattern_result["anomaly_types"] + ml_result["anomaly_types"])
                ),
                "severity": self._calculate_severity(
                    pattern_result, ml_result, final_anomaly
                ),
                "confidence": max(
                    pattern_result["confidence"], ml_result["confidence"]
                ),
    ...
    def _detect_ml_anomaly(self, data: Dict) -> Dict:
        ...
        try:
            ...
            features = {
                ...
                "Process temperature [K]": float(data.get("temperature", 300.0))
                + 10.0,  # Estimate
            ...
            if hasattr(self, "label_encoder") and self.label_encoder is not None:
                df_features["Type_encoded"] = self.label_encoder.transform(
                    df_features["Type"]
                )
                ...
                df_features["Type_encoded"] = (
                    df_features["Type"].map(type_mapping).fillna(1)
                )
                df_features = df_features.drop("Type", axis=1)
    ...
    def _calculate_severity(
        self, pattern_result: Dict, ml_result: Dict, final_anomaly: bool
    ) -> str:
...
def process_file(input_file: str, output_prefix: str = "output") -> Dict:
    ...
    print(
        f"Generated: {output_prefix}_results.json, {output_prefix}_summary.json, {output_prefix}_results.csv"
    )
